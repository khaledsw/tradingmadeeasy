...
class ControllerProductProduct extends Controller {
	private $error = array();
				...
				public function changeliveprcbrkdown() {
					...
					if($qty > 0) {
						...
						$result = $this->model_module_qty_based_pricing_breakdown->getqtyDataFromProductIdQty($product_id, $qty);
						if(isset($result['price_per_unit'])) {
							echo  $this->currency->format($result['price_per_unit'] * $qty);
						} else {
							$result = $this->model_module_qty_based_pricing_breakdown->getqtyDataFromProductIdQty($product_id, 1);
							if(isset($result['price_per_unit'])) {
								echo  $this->currency->format($result['price_per_unit'] * $qty);
							} else {
								echo 'nothing';
						...
						echo  $this->currency->format(0);
					}
				}
			
	public function index() {

				$data['qty_based_pricing_breakdown'] = $this->load->controller('module/qty_based_pricing_breakdown');
...
$data['custom_field'] = html_entity_decode($product_info['custom_field'], ENT_QUOTES, 'UTF-8');
