...
class ControllerMyocPod extends Controller {
	public function index($args) {
		$this->load->language('myoc/pod');

		$data['text_col_quantity'] = $this->language->get('text_col_quantity');
		$data['text_col_stock'] = $this->language->get('text_col_stock');
		$data['text_select'] = $this->language->get('text_select');
		$data['text_order'] = $this->language->get('text_order');
		$data['text_unit'] = $this->language->get('text_unit');
		$data['text_total'] = $this->language->get('text_total');
		$data['text_price'] = $this->language->get('text_price');
		$data['text_extax'] = $this->language->get('text_extax');
		...
		if($pods) {
			...
			foreach ($product_options as $product_option) {
				if($product_option['product_option_id'] == $args['product_option_id'] && is_array($product_option['product_option_value'])) {
					$option_discount_data =
					$product_option_value_data = array();
					$ranges = array();

					foreach ($product_option['product_option_value'] as $product_option_value) {
						$prev_pod = $product_option_value;
						...
						foreach ($quantities as $quantity) {
							foreach ($pods['pods'] as $pod) {
								if(isset($pod[$quantity]) && $pod[$quantity]['product_option_value_id'] == $product_option_value['product_option_value_id']) {
									$pod = $pod[$quantity];
									$prev_pod = $pod;
								} else {
									$pod = $prev_pod;
								...
								if($pod['calc_method'] == 'o' || $pod['calc_method'] == 'po') {
									$option_base_price = $product_option_value['price'];
						...
						if($pods['show_final']) {
							if($product_option_value['price_prefix'] == '+') {
								$product_option_value_price = ($product_info['special'] ? $product_info['special'] : $product_info['price']) + $product_option_value['price'];
							} else {								
								$product_option_value_price = ($product_info['special'] ? $product_info['special'] : $product_info['price']) - $product_option_value['price'];
							}
							$product_option_value['price_prefix'] = '';
						} else {
							$product_option_value_price = $product_option_value['price'];
						}
						...
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$product_option_value['price']) {
							if($pods['inc_tax'] == 'y' || $pods['inc_tax'] == 'both') {
								$option_price = $this->currency->format($this->tax->calculate($product_option_value_price, $product_info['tax_class_id'], $this->config->get('config_tax')));
							}
							if($pods['inc_tax'] == 'n' || $pods['inc_tax'] == 'both') {
								$option_extax = $this->currency->format($product_option_value_price);
							}
						}
						...
						$product_option_value_data[] = array(
							...
							'image'						=> isset($product_option_value['image']) ? $this->model_tool_image->resize($product_option_value['image'], 50, 50) : false,
							'price'						=> $option_price,
							'extax'						=> $option_extax,
							'stock'						=> $product_option_value['quantity'],
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/myoc/pod.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/myoc/pod.tpl', $data);
			} else {
				return $this->load->view('default/template/myoc/pod.tpl', $data);
			}		
		}
	}
}
