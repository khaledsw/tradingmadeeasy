	...
	public function getProducts() {
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				//  pricing breakdown
 					$product_pbp_query = $this->db->query("SELECT price_per_unit price FROM `" . DB_PREFIX . "qty_based_pricing_breakdown` bg left join " . DB_PREFIX . "qty_based_pricing_breakdown_product bgp on bg.qty_based_pricing_breakdown_id = bgp.qty_based_pricing_breakdown_id WHERE bgp.product_id = '" . (int)$product_id . "' AND ((bg.date_start = '0000-00-00' OR bg.date_start < NOW()) AND (bg.date_end = '0000-00-00' OR bg.date_end > NOW())) AND bg.status = '1' AND qty_from <= '" . (int)$discount_quantity . "' AND qty_to >= '" . (int)$discount_quantity . "' limit 1 ");

					if ($product_pbp_query->num_rows) {
						$price = $product_pbp_query->row['price'];
					}
