...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, 
				m.name AS manufacturer, (SELECT min(price_per_unit) price FROM " . DB_PREFIX . "qty_based_pricing_breakdown_product pbp left join " . DB_PREFIX . "qty_based_pricing_breakdown pb on pb.qty_based_pricing_breakdown_id = pbp.qty_based_pricing_breakdown_id WHERE pbp.product_id = p.product_id AND ((pb.date_start = '0000-00-00' OR pb.date_start < NOW()) AND (pb.date_end = '0000-00-00' OR pb.date_end > NOW())) AND pb.status = '1'  ORDER BY min(price_per_unit) desc LIMIT 1) AS pbp_discount, 
		...
		if ($query->num_rows) {
			return array(
				...
				'price'            => ($query->row['pbp_discount'] ? $query->row['pbp_discount'] : $query->row['price']),
