...
class ModelMyocPod extends Model {
	...
	public function installTable()
	{
		$this->upgradeTable();
  		...
  		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "myoc_pod_setting` (
			...
			`show_price_product` tinyint(1) NOT NULL DEFAULT '1',
			`show_price_cart` tinyint(1) NOT NULL DEFAULT '1',
			`show_final` tinyint(1) NOT NULL DEFAULT '0',
			`flat_rate` tinyint(1) NOT NULL DEFAULT '0',
			`flat_rate_weight` tinyint(1) NOT NULL DEFAULT '0',
  		...
  		$this->db->query("INSERT IGNORE INTO `" . DB_PREFIX . "myoc_pod_setting` (
			...
			`stock_column`,
			`qty_cart`)
			SELECT `product_option_id`,1,1,0,0,0,0,'y','h','total',1,'single',0,0 FROM `" . DB_PREFIX . "product_option`;");
	}
	...
	public function upgradeTable()
	{
		...
		if($query->num_rows)
		{
			...
			if($status) {
				$this->db->query("ALTER TABLE `" . DB_PREFIX . "myoc_pod_setting` ADD `qty_format` enum('single','range') COLLATE utf8_unicode_ci NOT NULL DEFAULT 'single',
					ADD `stock_column` tinyint(1) NOT NULL DEFAULT '0' AFTER `qty_column`");
			}
		}
		...
		if($query->num_rows)
		{
			$query = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "myoc_pod_setting`;");
			foreach ($query->rows as $column_data) {
				if($column_data['Field'] == 'flat_rate_weight')
				{
					$status = false;
					break;
				}
			...
			if($status) {
				$this->db->query("ALTER TABLE `" . DB_PREFIX . "myoc_pod_setting` ADD `flat_rate_weight` tinyint(1) NOT NULL DEFAULT '0' AFTER `flat_rate`");
			}
		}
	}
}
