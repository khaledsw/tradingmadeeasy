...
class ModelModuleExcelport extends Model {
	...
	public $exportData = array(
        ...
        'Manufacturers' => array(
            ...
            'keyword' => "SELECT SUBSTRING(query, 12) as manufacturer_id, keyword as value FROM {DB_PREFIX}url_alias ua WHERE ua.query LIKE 'manufacturer_id=%'",
        )
	);
	...
	public function importXLS($type, $language, $file = '', $settings, $addAsNew = false) {
		...
		switch ($type) {
            ...
            case 'Manufacturers' : {
                $this->load->model('module/excelport_manufacturer');
                $this->model_module_excelport_manufacturer->importXLSManufacturers($file, $settings['ImportLimit']);
            } break;
		}
	}
	public function exportXLS($type, $language, $store, $destinationFolder = '', $settings, $quickExport = false, $filter = false, $filters = array()) {
		...
		switch ($type) {
            ...
            case 'Manufacturers' : {
                $this->load->model('module/excelport_manufacturer');
                $this->model_module_excelport_manufacturer->exportXLSManufacturers($store, $destinationFolder, $settings['ExportLimit'], !empty($filters['Manufacturers']) && $filter ? $filters['Manufacturers'] : array());   
            } break;
		}
	}
}
